 ...
 * */
static int32_t tunneler_cmp(const void *key, const void *with)
{
  return ((corridor_path_t *)key)->cost - ((corridor_path_t *)with)->cost;
}

static void tunneler(dungeon_t *d)
{
  static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  ...
  heap_init(&h, tunneler_cmp, NULL);

  for (y = 0; y < DUNGEON_Y; y++)
  {
    for (x = 0; x < DUNGEON_X; x++)
    {
      if (mapxy(x, y) == ter_wall_immutable || d->hardness[y][x] < 255)
      {
        path[y][x].hn = heap_insert(&h, &path[y][x]);
      }
      else
  ...
  while ((p = heap_remove_min(&h)))
  {
    ...
    int weight = 1 + (d->hardness[p->pos[dim_y]][p->pos[dim_x]] / 85);
  ...
  for (y = 0; y < DUNGEON_Y; y++)
  {
    for (x = 0; x < DUNGEON_X; x++)
    {
      ...
      else if (mapxy(x, y) != ter_wall_immutable)
      {
...
static void non_tunneler(dungeon_t *d)
{
  ...
  while ((p = heap_remove_min(&h)))
  {
    ...
    //need to add if statements for corners
  ...
  for (y = 0; y < DUNGEON_Y; y++)
  {
    for (x = 0; x < DUNGEON_X; x++)
    {
      ...
      else if (mapxy(x, y) == ter_floor || mapxy(x, y) == ter_floor_hall || mapxy(x, y) == ter_floor_room || mapxy(x, y) == ter_stairs || mapxy(x, y) == ter_stairs_down || mapxy(x, y) == ter_stairs_up)
      {
        printf("%d", path[y][x].cost % 10);
      }
      else
