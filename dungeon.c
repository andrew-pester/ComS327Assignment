...
/**
 * used to see if monster and player are in the same room.
 **/
uint32_t in_room(dungeon_t *d, int16_t y, int16_t x, int16_t yM, int16_t xM)
{
  ...
  for (i = 0; i < d->num_rooms; i++)
  {
    if ((x >= d->rooms[i].position[dim_x]) &&
        ...
        (xM >= d->rooms[i].position[dim_x]) &&
        (xM < (d->rooms[i].position[dim_x] + d->rooms[i].size[dim_x])) &&
        (yM >= d->rooms[i].position[dim_y]) &&
        (yM < (d->rooms[i].position[dim_y] + d->rooms[i].size[dim_y])))
    {
      return 1;
    }
  }
...
void render_dungeon(dungeon_t *d)
{
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++)
  {
    ...
    {
      if (d->monster[p[dim_y]][p[dim_x]].pos[dim_x] == p[dim_x] && d->monster[p[dim_y]][p[dim_x]].pos[dim_y] == p[dim_y] && d->monster[p[dim_y]][p[dim_x]].type == '@')
      {
        putchar('@');
      }
      else
...
void generate_monsters(dungeon_t *d, int nummon)
{
  for (int i = 0; i < nummon; i++)
  {
    ...
    while (!done)
    {
      x = rand() % 80;
      y = rand() % 21;
      d->monster[y][x].speed = rand() % 16 + 5;
      //monsters can spawn inside of rooms but not on the player
      if (d->map[y][x] == ter_floor_room && d->pc.position[dim_y] != y && d->pc.position[dim_x] != x && d->monster[y][x].type == ' ')
      {
        done = 1;
      }
    }
    ...
    d->monster[y][x].id = i;
    d->monster[y][x].pos[dim_y] = y;
    d->monster[y][x].pos[dim_x] = x;
    d->monster[y][x].turn = 0;
    d->monster[y][x].last_known[dim_x] = -1;
    d->monster[y][x].last_known[dim_y] = -1;
  }
}
