...
static uint32_t io_ranged_attack(dungeon *d)
{
  ...
  //essentially just move's attacking and inspecting monsters mashed together
  pair_t dest, tmp;
  int c;
  fd_set readfs;
  struct timeval tv;
  ...
  io_display(d);
  if (d->PC->eq[eq_slot_ranged]->get_type() == objtype_WAND)
  {
    mvprintw(0, 0, "Press r to cast a bolt spell or c to cast a poison ball spell (ESC to cancel).");
  }
  else
  {
    mvprintw(0, 0, "Press r to shoot your bow or c to shoot a arrow storm (ESC to cancel).");
  }
    ...
    for (; tmp[dim_y] < dest[dim_y] + 2; tmp[dim_y]++)
    {
      for (tmp[dim_x] = dest[dim_x] - 1; tmp[dim_x] < dest[dim_x] + 2; tmp[dim_x]++)
      {
        if (charpair(tmp) && charpair(tmp) != d->PC)
        {
          ...
          {
            io_queue_message("%s%s dies.", is_unique(charpair(tmp)) ? "" : "The ", charpair(tmp)->name);
            character_increment_dkills(d->PC);
            character_increment_ikills(d->PC, (character_get_dkills(charpair(tmp)) +
                                               character_get_ikills(charpair(tmp))));
            d->num_monsters--;
            charpair(tmp)->hp = 0;
            charpair(tmp)->alive = 0;
            charpair(tmp) = NULL;
          }
          else
          {
